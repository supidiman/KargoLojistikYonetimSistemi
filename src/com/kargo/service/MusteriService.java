...
catch(Exception ex) 
{
	...
	public void MusteriSil(String kimlik) 
	{
		if(kimlik == null || kimlik.trim().isEmpty()) {
			throw new IllegalArgumentException("Silinecek müşterinin TC kimlik numarası boş olamaz!");
		}
		try 
		{
			musteridao.sil(kimlik);
			System.out.println("basarılı");
			
		}catch(Exception ex) {
			throw new RuntimeException("VHATA  SİLİNMEDE: " + ex.getMessage());
		}
	}
